 ...
 */
               ...
               int j = 20;
               int n = 40-j;
               int k = random.nextInt(n)+j;//il numero di chiavi è compreso tra 20 e 40
 ...
 */
         ...
         try{
            ...
            while(riga1 != null){
               ...
               albero.insertRic(Integer.parseInt(data[0])); //inserisce nell'albero la chiave
               ...
               for (int i=2;i<=nodiLista;i++ ) {//legge tutti gli elementi della lista e li inserisce nella lista del nodo corrispondente
                  ...
                  albero.inserisciCodiceLista(albero.searchRic(Integer.parseInt(data[0]))
                     ,Integer.parseInt(data[i])); //inserisce nella lista relativa al nodo i codici
               }
 ...
 */
             ...
             try{
               ...
               while(riga2 != null){
                  ...
                  switch(data2[0]){
                     case "I":
                        albero.insertRic(Integer.parseInt(data2[1])); //inserisce la chiave nell'albero
                        ...
                        for (int i=3;i<=nodiLista;i++ ) {//legge tutti gli elementi della lista e li inserisce nella lista del nodo corrispondente
                           albero.inserisciCodiceLista(albero.searchRic(Integer.parseInt(data2[1]))
                           ,Integer.parseInt(data2[i]));
                        }
                        System.out.println("Operazioni elementari inserimento= ");
                        albero.printElemOp();
                        break;


                     case "R":
                        if ( albero.searchRic(Integer.parseInt(data2[1])) != null) {//se il nodo esiste
                           outputStream5.print("Chiave: "+Integer.parseInt(data2[1]));
                           outputStream5.println("\tCodici lista: "+albero.searchRic(Integer.parseInt(data2[1])).getCodici());
                           System.out.println("Ricerca avvenuta con successo");
                        } else{
                           outputStream5.println("Chiave non trovata");
                        }
                        System.out.println("Operazioni elementari ricerca= ");
                        albero.printElemOp();
                        break;

                     case "E":
                        albero.elimina(Integer.parseInt(data2[1]));
                        System.out.println("nodo eliminato");
                        outputStream5.println("Cancellazione riuscita");
                        System.out.println("Operazioni elementari eliminazione= ");
                        albero.printElemOp();
                        break;

                     case "RR":
                       int codiceLista= albero.ricercaCodice(albero.searchRic(Integer.parseInt(data2[1]))
                           ,Integer.parseInt(data2[2]));

                          if (codiceLista==1) {
                             outputStream5.println("Elemento trovato");
                          } else if (codiceLista==0){
                           outputStream5.println("Elemento NON trovato");
                          } else{
                           outputStream5.println("Chiave non trovata");
                          }
                          System.out.println("Operazioni elementari di ricerca nella lista=");
                          albero.printElemOp();
                          albero.printElemOpLista();
